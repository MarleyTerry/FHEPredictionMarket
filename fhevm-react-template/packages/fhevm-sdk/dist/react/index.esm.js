...
const FhevmContext = createContext(undefined);
/**
...
function useEncryption() {
    ...
    const [error, setError] = useState(null);
    const encrypt = useCallback(async (type, value) => {
        if (!client || !isReady) {
    ...
    const encryptBatch = useCallback(async (contractAddress, userAddress, values) => {
        if (!client || !isReady) {
            setError(new Error('FHEVM client not ready'));
            return null;
        }
    ...
    const encryptBool = useCallback((value) => encrypt('bool', value), [encrypt]);
    const encryptUint8 = useCallback((value) => encrypt('uint8', value), [encrypt]);
    const encryptUint16 = useCallback((value) => encrypt('uint16', value), [encrypt]);
    const encryptUint32 = useCallback((value) => encrypt('uint32', value), [encrypt]);
    const encryptUint64 = useCallback((value) => encrypt('uint64', value), [encrypt]);
    return {
        encrypt,
        encryptBatch,
        encryptBool,
...
function useDecryption(signer, gatewayUrl) {
    ...
    const [error, setError] = useState(null);
    const decrypt = useCallback(async (type, params) => {
        if (!signer) {
    ...
    const publicDecrypt = useCallback(async (contractAddress, ciphertext) => {
        if (!signer) {
            setError(new Error('Signer not provided'));
            return null;
        }
    ...
    const decryptBool = useCallback((params) => decrypt('bool', params), [decrypt]);
    const decryptUint8 = useCallback((params) => decrypt('uint8', params), [decrypt]);
    const decryptUint16 = useCallback((params) => decrypt('uint16', params), [decrypt]);
    const decryptUint32 = useCallback((params) => decrypt('uint32', params), [decrypt]);
    const decryptUint64 = useCallback((params) => decrypt('uint64', params), [decrypt]);
    return {
        decrypt,
        publicDecrypt,
        decryptBool,
...
function useContract(config) {
    ...
    const estimateGas = useCallback(async (method, args = []) => {
        if (!contract) {
            setError(new Error('Contract not initialized'));
            return null;
        }
...
function useEncryptedTransaction(config) {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const sendEncryptedTransaction = useCallback(async (params) => {
        if (!client || !isReady) {
...
export { FhevmProvider, useContract, useDecryption, useEncryptedTransaction, useEncryption, useFhevmClient, useFhevmContext };
